...
microbeTreePath <- 'raw_data/species_rooted_mapped_labels.tree' #microbial phylogeny
...
fulltablePath <- 'raw_data/species_table.txt' #table of symportal profiles
divtablePath <- 'raw_data/175_43.DIVs.counts.txt' #table of symportal divs
...
} else if(!is.ultrametric(microbeTree.root)) {
...
divtable <- read.table(divtablePath,
                       header = T,
                       sep = '\t',
                       comment.char = '',
                       row.names = 1,
                       check.names = F)
newtable <- as.matrix(divtable[rownames(fulltable)[rownames(fulltable) %in% rownames(divtable)], -ncol(divtable)]) # last column in divs file is not named
mode(newtable) <- 'numeric'
##

## link profiles to sequence variants
profileDefs <- sapply(fulltable[nrow(fulltable),], function(x) {
    profs <- strsplit(x, '; ')[[1]]
    return(strsplit(profs[length(profs)], '/|-'))
})
profileDefs <- profileDefs[names(profileDefs) %in% microbeTree.root$tip.label]
...
y.old <- newtable[idx, colnames(newtable) %in% unlist(profileDefs)]
...
NSamples <- nrow(present)

profileMat <- profileMat[rowSums(profileMat) > 0,]
finalMicrobeTree <- drop.tip(microbeTree.root,
                             microbeTree.root$tip.label[!microbeTree.root$tip.label %in% rownames(profileMat)])
profileMat <- profileMat[finalMicrobeTree$tip.label, colnames(present)]
NSeqs <- ncol(profileMat)
##

## generate some summary numbers regarding microbes
microbeTips <- finalMicrobeTree$tip.label
...
newermap <- newermap[rownames(present),]
##

## prepare sequencing depth input
newermap$sequencing_depth <- rowSums(y.old[, colnames(y.old) %in% unlist(profileDefs[names(profileDefs) %in% microbeTips])])
