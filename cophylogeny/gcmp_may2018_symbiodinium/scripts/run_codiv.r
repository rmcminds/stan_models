...
hostOUAlphaPriorExpect <- 0.5 #uniform 0,1 prior on d = exp(-2 * alpha)
microbeOUAlphaPriorExpect <- 0.5 #uniform 0,1 prior on d = exp(-2 * alpha)
stDLogitHostPriorExpect <- 0.05
stDLogitMicrobePriorExpect <- 0.05
...
thin <- 2^(1) ## NIterations / thin number of Monte Carlo samples from the fit
##
...
for(i in 1:NTrees) {
    ...
    hostTreesSampled[[i]] <- ladderize(drop.tip(hostTree[[hostTreeSampleNumbers[[i]]]], hostTree[[hostTreeSampleNumbers[[i]]]]$tip.label[!hostTree[[hostTreeSampleNumbers[[i]]]]$tip.label %in% levels(sampleMap[[i]][,sampleTipKey])]))
