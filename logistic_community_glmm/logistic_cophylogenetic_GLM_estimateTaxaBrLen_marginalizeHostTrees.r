...
modelfile <- 'logistic_cophylogenetic_GLM_estimateTaxaBrLen_marginalizeHostTrees.stan'
...
## Stan options
nchains <- 4 ## this is per tree; since I'm doing a large number of trees in parallel i'll just do one chain for each
...
standat <- list(NEstSamples=NEstSamples, NObs=NObs, NEstTaxNodes=NEstTaxNodes, NEstTips=NEstTips, NFactors=NFactors, NEffects=NEffects, present=present, samplenames=samplenames, tipnames=tipnames, factLevelMat=factLevelMat, ancestors=ancestors, modelMat=modelMat, hostAncestors=hostAncestorsArray, edgetobin=edgetobinArray, NHostNodes=NHostNodes, NTimeBins=NTimeBins, taxAveStDPriorExpect=taxAveStDPriorExpect, timeBinSizes=timeBinSizesArray, NTrees=nparalleltrees)
...
## logistic regression
##
fit <- stan(file=modelfile, data=standat, control=list(adapt_delta=adapt_delta, max_treedepth=max_treedepth), iter=iterations, thin=max(c(floor(iterations * nchains / 5000),1)), chains=nchains )
...
vars1 <- extract(fit, pars='stDProps')[[1]]
...
relativeEvolRatesADiv <- extract(fit, pars='relativeEvolRatesADiv')[[1]]
...
relativeEvolRatesSpec <- extract(fit, pars='relativeEvolRatesSpec')[[1]]
