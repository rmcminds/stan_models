...
parameters {
    row_vector<lower=0>[NEstTaxNodes] taxNodeScalesRaw[NTrees];
...
transformed parameters {
    row_vector[NEstTaxNodes] taxNodeScales[NTrees];
	...
	for (t in 1:NTrees) {
        taxNodeScales[t]
            = sqrt(square(taxNodeScalesRaw[t])
                   / mean(square(taxNodeScalesRaw[t]) * ancestors));
		scales[t]
            = sqrt((2 * NFactors + 1)
                   * stDPropList[t])
              * aveStD[t];
        newEdgesADiv[t]
            = edgetobin[t] * timeBinPropsADiv[t];
        hostNodeScalesADiv[t]
            = sqrt(newEdgesADiv[t]) * scales[t][NFactors];
        scaledAlphaDivEffects[t]
            = append_row(globalIntercept[t],
                append_row(factLevelMat[2:NEffects, 2:NFactors]
                           * segment(scales[t], 1, NFactors - 1),
                           hostNodeScalesADiv[t])
                .* rawAlphaDivEffects[t]);
        newEdgesSpec[t]
            = edgetobin[t] * timeBinPropsSpec[t];
        hostNodeScalesSpec[t]
            = sqrt(newEdgesSpec[t]) * scales[t][2 * NFactors + 1];
        scaledTaxNodeEffects[t]
            = diag_post_multiply(
                diag_pre_multiply(
                    append_row(factLevelMat * segment(scales[t], NFactors + 1, NFactors),
                        hostNodeScalesSpec[t]),
                    rawTaxNodeEffects[t]),
                taxNodeScales);
	}
...
model {
	...
	for (t in 1:NTrees) {
        taxNodeScalesRaw[t] ~ exponential(1);
	...
	for (t in 1:NTrees)
		logit_ratios[t] = append_col(modelMat, hostAncestors[t])
                          * (rep_matrix(scaledAlphaDivEffects[t], NEstTips)
                             + scaledTaxNodeEffects[t]
                               * ancestors);
	for (n in 1:NObs) {
		vector[NTrees] lps;
		for(t in 1:NTrees) {
			lps[t] = logTreeProbs[t]
                     + bernoulli_logit_lpmf(present[n]
                         | logit_ratios[t][samplenames[n], tipnames[n]]);
		}
