...
outdir <- 'logistic_cophylogenetic_GLM_marginalizeHostTrees_out/'
fulltable <- t(read.table('species_table.txt', header=T, sep='\t', comment.char='', row.names=1, check.names=F))
modelfile <- 'logistic_cophylogenetic_GLM_marginalizeHostTrees.stan'
...
timeBinSizesArray <- array(NA_real_, dim = c(nparalleltrees, NTimeBins))
edgetobinArray <- array(NA_real_, dim = c(nparalleltrees, NHostNodes, NTimeBins))
hostAncestorsArray <- array(NA_real_, dim = c(nparalleltrees, NEstSamples, NHostNodes))
for(i in 1:nparalleltrees) {
    timeBinSizesArray[i,] <- relativeTimeBinSizes[[i]]
	edgetobinArray[i,,] <- edgetobin[[i]]
	hostAncestorsArray[i,,] <- hostAncestorsExpanded[[i]]
}

standat <- list(NEstSamples=NEstSamples, NObs=NObs, NEstTaxNodes=NEstTaxNodes, NEstTips=NEstTips, NFactors=NFactors, NEffects=NEffects, present=present, samplenames=samplenames, tipnames=tipnames, factLevelMat=factLevelMat, ancestors=ancestors, modelMat=modelMat, hostAncestors=hostAncestorsArray, edgetobin=edgetobinArray, NHostNodes=NHostNodes, NTimeBins=NTimeBins, taxAveStDPriorExpect=taxAveStDPriorExpect, timeBinSizes=timeBinSizesArray, taxNodeScales=taxNodeScales, NTrees=nparalleltrees)

## logistic regression
##
fit <- stan(file=modelfile, data=standat, control=list(adapt_delta=adapt_delta, max_treedepth=max_treedepth), iter=iterations, thin=floor(iterations * nchains / 5000), chains=nchains )
