...
modelfile <- 'logistic_cophylogenetic.stan'
...
taxNodeScales <- sqrt(bacttree.root.chronos.Y.filtered$edge.length)

allnodes <- unique(bacttree.root.chronos.Y.filtered$edge[,1])
NSamplesRaw <- nrow(yb)
ancestors <- matrix(0, NEstTaxNodes + 1, NEstTips)
for(tip in 1:NEstTips) {
    ancestors[, tip] <- as.numeric(1:(NEstTaxNodes + 1) %in% c(Ancestors(bacttree.root.chronos.Y.filtered, tip), tip))
}
##
...
for(i in 1:nparalleltrees) {
    ...
    sums <- summary(fit[[i]], pars='scaledADivAndTaxNodeEffects', probs=c(0.05,0.95), use_cache = F)

    sums3d <- array(NA, dim=c(NEffects + NHostNodes, NEstTaxNodes + 1, ncol(sums$summary)))
    for(effect in 1:(NEffects + NHostNodes)) {
        sums3d[effect,,] <- sums$summary[(effect-1) * (NEstTaxNodes + 1) + (1:(NEstTaxNodes + 1)),]
    }
    dimnames(sums3d) <- list(c(colnames(modelMat)[1:NEffects],colnames(hostAncestors[[i]])), c('ADiv',rownames(ancestors)), colnames(sums$summary))
    ...
    effectList[[i]] <- extract(fit[[i]], pars='scaledADivAndTaxNodeEffects')[[1]]

    for(j in sumconts) {
        hi <- effectList[[i]][,factLevelMat[,j] == 1,]
        sup <- -apply(hi, 1, colSums)
        rownames(sup) <- c('ADiv',rownames(ancestors))
        sup2 <- sup
	...
	currtabledir <- paste0(outdir,'tree_',i,'/tables/tips/')
    dir.create(currtabledir, recursive=T)
