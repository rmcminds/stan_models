...
modelfile <- 'logistic_cophylogenetic_GLM_estimateTaxaBrLen.stan'
...
nparalleltrees <- 10 ## number of random trees to sample and to fit the model to
...
y.old <- newtable[idx,colnames(newtable) %in% bacttree.root$tip.label]
newermap <- newmap[idx,]
bacttree.root.Y <- drop.tip(bacttree.root,bacttree.root$tip.label[!bacttree.root$tip.label %in% colnames(y.old)])



## summarize the putative taxa to be estimated
taxa <- factor(colnames(y.old),levels=bacttree.root.Y$tip.label)
...
bacttree.root.Y.filtered <- drop.tip(bacttree.root.Y, bacttree.root.Y$tip.label[!bacttree.root.Y$tip.label %in% esttips])

NEstIntTaxNodes <- bacttree.root.Y.filtered$Nnode
NEstTaxNodes <- NEstTips + NEstIntTaxNodes - 1

allnodes <- unique(bacttree.root.Y.filtered$edge[,1])
NSamplesRaw <- nrow(yb)
ancestors <- matrix(0, NEstTaxNodes + 1, NEstTips)
for(tip in 1:NEstTips) {
    ancestors[, tip] <- as.numeric(1:(NEstTaxNodes + 1) %in% c(Ancestors(bacttree.root.Y.filtered, tip), tip))
}
##


colnames(ancestors) <- paste0('t',colnames(yb))
rownames(ancestors) <- paste0('i',1:(NEstTaxNodes + 1))
rownames(ancestors)[1:NEstTips] <- colnames(ancestors)

ancestors <- ancestors[-(NEstTips + 1),]
...
for (i in 1:nparalleltrees) {
	hostAncestors[[i]] <- matrix(0, NHostTips, NHostNodes+1)
	for(tip in 1:NHostTips) {
	    hostAncestors[[i]][tip, ] <- as.numeric(1:(NHostNodes+1) %in% c(Ancestors(hosttreesSampled[[i]], tip), tip))
	}
    rownames(hostAncestors[[i]]) <- hosttreesSampled[[i]]$tip.label
    colnames(hostAncestors[[i]]) <- paste0('i',1:(NHostNodes+1))
    colnames(hostAncestors[[i]])[1:NHostTips] <- rownames(hostAncestors[[i]])
    hostAncestors[[i]] <- hostAncestors[[i]][, -(NHostTips + 1)]
...
standat <- list()
for (i in 1:nparalleltrees) {
    standat[[i]] <- list(NEstSamples=NEstSamples, NObs=NObs, NEstTaxNodes=NEstTaxNodes, NEstTips=NEstTips, NFactors=NFactors, NEffects=NEffects, present=present, samplenames=samplenames, tipnames=tipnames, factLevelMat=factLevelMat, ancestors=ancestors, modelMat=modelMat, hostAncestors=hostAncestorsExpanded[[i]], edgetobin=edgetobin[[i]], NHostNodes=NHostNodes, NTimeBins=NTimeBins, taxAveStDPriorExpect=taxAveStDPriorExpect, timeBinSizes=relativeTimeBinSizes[[i]])
}
...
## logistic regression
##
fit <- mclapply(1:nparalleltrees, function(i) stan(file=modelfile, data=standat[[i]], control=list(adapt_delta=adapt_delta, max_treedepth=max_treedepth), iter=iterations, thin=max(c(floor(iterations/2000),1)), chains=nchains ))
...
for(i in 1:nparalleltrees) {
    ...
    colnames(vars1) <- c(paste0('ADiv.',c(colnames(factLevelMat)[2:NFactors],'host')), c(colnames(factLevelMat),'host'))
    ...
    timeBinPropsADiv <- extract(fit[[i]], pars='timeBinPropsADiv')[[1]]
    
    pdf(file=paste0(currplotdir,'timeBinPropsADiv_boxes.pdf'), width=25, height=15)
    ...
    timeBinPropsSpec <- extract(fit[[i]], pars='timeBinPropsSpec')[[1]]
    
    pdf(file=paste0(currplotdir,'timeBinPropsSpec_boxes.pdf'), width=25, height=15)
    ...
    relativeEvolRatesADiv <- extract(fit[[i]], pars='relativeEvolRatesADiv')[[1]]
    colnames(relativeEvolRatesADiv) <- c(paste0('before ',meanBoundariesRounded[1],' mya'), paste0(meanBoundariesRounded[1],' - ',meanBoundariesRounded[2],' mya'), paste0(meanBoundariesRounded[2],' - ',meanBoundariesRounded[3],' mya'), paste0(meanBoundariesRounded[3],' - ',meanBoundariesRounded[4],' mya'), paste0(meanBoundariesRounded[4],' - present'))
    
    pdf(file=paste0(currplotdir,'evolRatesADivRelToWeightedMean.pdf'), width=25, height=15)
    boxplot(relativeEvolRatesADiv, xlab='Time Period', ylab='Rate of Evolution of Alpha Diversity Relative to Weighted Mean')
    graphics.off()
    
    save(relativeEvolRatesADiv,file=paste0(currdatadir,'relativeEvolRatesADiv.RData'))
    
    relativeEvolRateADiv4p5 <- (relativeEvolRatesADiv[,4] * timeBinSizes[[1]][[4]] + relativeEvolRatesADiv[,5] * timeBinSizes[[1]][[5]]) / (timeBinSizes[[1]][[4]] + timeBinSizes[[1]][[5]])
    relativeEvolRatesADivMerged45 <- cbind(relativeEvolRatesADiv[,-c(4,5)],relativeEvolRateADiv4p5)
    colnames(relativeEvolRatesADiv)[[4]] <- paste0(meanBoundariesRounded[3],' - present')
    
    pdf(file=paste0(currplotdir,'evolRatesADivRelToWeightedMeanMerged45.pdf'), width=25, height=15)
    boxplot(relativeEvolRatesADivMerged45, xlab='Time Period', ylab='Rate of Evolution Relative to Weighted Mean')
    graphics.off()
    
    pdf(file=paste0(currplotdir,'logEvolRatesADivRelToWeightedMeanMerged45.pdf'), width=25, height=15)
    boxplot(log(relativeEvolRatesADivMerged45), xlab='Time Period', ylab='Log Rate of Evolution Relative to Weighted Mean')
    graphics.off()
    
    
    relativeEvolRatesSpec <- extract(fit[[i]], pars='relativeEvolRatesSpec')[[1]]
    colnames(relativeEvolRatesSpec) <- colnames(relativeEvolRatesADiv)
    
    pdf(file=paste0(currplotdir,'evolRatesSpecRelToWeightedMean.pdf'), width=25, height=15)
    boxplot(relativeEvolRatesSpec, xlab='Time Period', ylab='Rate of Evolution of Specificity Relative to Weighted Mean')
    graphics.off()
    ...
    sums <- summary(fit[[i]], pars='scaledTaxNodeEffects', probs=c(0.05,0.95), use_cache = F)
    
    sums3d <- array(NA, dim=c(NEffects + NHostNodes, NEstTaxNodes, ncol(sums$summary)))
    for(effect in 1:(NEffects + NHostNodes)) {
        sums3d[effect,,] <- sums$summary[(effect-1) * (NEstTaxNodes) + (1:(NEstTaxNodes)),]
    }
    dimnames(sums3d) <- list(c(colnames(modelMat)[1:NEffects],colnames(hostAncestors[[i]])), rownames(ancestors), colnames(sums$summary))
    ...
    effectList[[i]] <- extract(fit[[i]], pars='scaledTaxNodeEffects')[[1]]
    
    for(j in sumconts) {
        hi <- effectList[[i]][,factLevelMat[,j] == 1,]
        sup <- -apply(hi, 1, colSums)
        rownames(sup) <- rownames(ancestors)
        sup2 <- sup
        dim(sup2) <- c(dim(sup),1)
        yeah <- monitor(aperm(sup2,c(2,3,1)), warmup=0, probs=c(0.05,0.95))
        rownames(yeah) <- rownames(sup)
        cat('\t', file=paste0(currtabledir,j,levels(newermap[,j])[nlevels(newermap[,j])],'.txt'))
        write.table(yeah, file=paste0(currtabledir,j,levels(newermap[,j])[nlevels(newermap[,j])],'.txt'), sep='\t', quote=F,append=T)
    }
}
...
save(effectList,file=paste0(currdatadir,'scaledTaxNodeEffects.RData'))
...
timeBinPropsADiv <- extract(allfit, pars='timeBinPropsADiv')[[1]]

pdf(file=paste0(currplotdir,'timeBinPropsADivboxes.pdf'), width=25, height=15)
boxplot(timeBinPropsADiv, cex.axis=0.5, las=2)
graphics.off()
...
relativeEvolRatesADiv <- extract(allfit, pars='relativeEvolRatesADiv')[[1]]
colnames(relativeEvolRatesADiv) <- c(paste0('before ',meanBoundariesRounded[1],' mya'), paste0(meanBoundariesRounded[1],' - ',meanBoundariesRounded[2],' mya'), paste0(meanBoundariesRounded[2],' - ',meanBoundariesRounded[3],' mya'), paste0(meanBoundariesRounded[3],' - ',meanBoundariesRounded[4],' mya'), paste0(meanBoundariesRounded[4],' - present'))

pdf(file=paste0(currplotdir,'evolRatesADivRelToWeightedMean.pdf'), width=25, height=15)
boxplot(relativeEvolRatesADiv, xlab='Time Period', ylab='Rate of Evolution Relative to Weighted Mean')
graphics.off()
